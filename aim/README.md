# ZeroDefect — Phase 2: AIM Edge Deployment

## Required Hardware
| Component | Part # / Notes |
|---|---|
| AIM Board | ZeroDefect AIM Board (ESP32-CAM compatible, camera included) |
| USB-to-TTL Adapter | CP2102 or CH340 (for flashing) |
| Jumper wires | For IO0 flash mode |
| 5V Power Supply | 2A minimum |

## Why AIM?
The AIM IoT device integrates a 2MP OV2640 camera, 4MB PSRAM, and Wi-Fi/BT in a highly compact module. It runs Arduino/ESP-IDF or MicroPython and hosts a TCP web server — making it the easiest path to a standalone inspection unit.
...
- AIM captures frame, sends JPEG over Wi-Fi to laptop
- Laptop runs inference, returns decision to AIM
- AIM displays result on web page
- Use this if quantized model doesn't fit in AIM memory

## Model Conversion Pipeline
```
Phase 1 (PyTorch) → ONNX → TFLite INT8 → AIM Flash
```

Run conversion:
```bash
# Step 1: Export to ONNX
python aim/quantize_model.py --export-onnx

# Step 2: Convert ONNX → TFLite (requires tensorflow)
pip install tensorflow
python aim/quantize_model.py --export-tflite

# Step 3: Report memory budget
python aim/quantize_model.py --report
```

## ⚠️ HARDWARE TESTING REQUIRED
> The following steps require physical access to an AIM board and CANNOT be validated in software only:
...
1. Connect AIM to USB-TTL: RX→TX, TX→RX, GND→GND, 5V→5V
2. Bridge IO0 to GND (flash mode)
3. In Arduino IDE: Tools → Board → ESP32 → AI Thinker ESP32-CAM (since AIM is compatible)
4. Upload `aim/firmware/ZeroDefect_AIM.ino`
...
## Fallback Plan
If the quantized model does not fit: implement Option B (edge-assist). The laptop runs `src/live_demo.py` which also serves an HTTP endpoint. AIM sends frames to `http://laptop-ip:8000/api/infer` and displays the result.
